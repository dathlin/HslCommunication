...
namespace HslCommunicationDemo.HslDebug
{
	public partial class FormCheck : HslFormContent
	{
		...
		private byte[] GetFcs( byte[] source, int left, int right )
		{
			...
			byte[] fcs = new byte[2];
			fcs[0] = SoftBasic.BuildAsciiBytesFrom( (byte)tmp )[0];
			fcs[1] = SoftBasic.BuildAsciiBytesFrom( (byte)tmp )[1];
		...
		private void button_fcs_calcu_Click( object sender, EventArgs e )
		{
			...
			byte[] result = GetFcs( source, left, right );
		...
		private void button_lrc_calcu_Click( object sender, EventArgs e )
		{
			...
			byte[] result          = new byte[] { GetLRC( source, left, right ) };
		...
		private void button_lrc_check_Click( object sender, EventArgs e )
		{
			...
			byte[] lrc = new byte[] { GetLRC( source, left, right ) };
