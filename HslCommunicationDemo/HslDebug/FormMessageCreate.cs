...
namespace HslCommunicationDemo.HslDebug
{
	public partial class FormMessageCreate : System.Windows.Forms.Form
	{
		...
		private void FormMessageCreate_Load( object sender, EventArgs e )
		{
			...
			{
				OperateResult<List<byte[]>> command = MelsecFxLinksHelper.BuildReadCommand( melsec2, address, length, true );
			...
			( address, length ) =>
			{
				OperateResult<List<byte[]>> command = MelsecFxLinksHelper.BuildReadCommand( melsec2, address, length, false );
				if (!command.IsSuccess) return OperateResult.CreateFailedResult<byte[]>( command );
