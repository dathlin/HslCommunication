...
namespace HslCommunicationDemo
{
    public partial class FormInovanceSerialOverTcp : HslFormContent
    {
        ...
        {
            OperateResult<byte[]> read = inovance.ReadFromCoreServer( HslCommunication.BasicFramework.SoftBasic.HexStringToBytes( textBox13.Text ) );
            if (read.IsSuccess)
            {
        ...
        private void thread_test2( )
        {
            ...
            while (count > 0)
            {
                if (!inovance.Write( "100", (short)1234 ).IsSuccess) failed++;
                if (!inovance.ReadInt16( "100" ).IsSuccess) failed++;
                count--;
            }
            thread_end( );
        }
