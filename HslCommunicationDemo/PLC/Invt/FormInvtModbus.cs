...
{
	public partial class FormInvtModbus : HslFormContent
	{
		public FormInvtModbus( )
		{
			InitializeComponent( );
		}
		...
		private void button1_Click( object sender, EventArgs e )
		{
			...
			if (comboBox_series.SelectedIndex == 0)
				busTcpClient.RegisteredAddressMapping( ModbusMappingAddress.Invt_Ts );  // 注册维控的Modbus地址
			...
			try
			{
				...
				if (connect.IsSuccess)
				{
					...
					codeExampleControl.SetCodeText( busTcpClient, nameof( busTcpClient.Station ), nameof( busTcpClient.AddressStartWithZero ), nameof( busTcpClient.IsCheckMessageId ),
						nameof( busTcpClient.IsStringReverse ), nameof( busTcpClient.DataFormat ), "HslCommunication.ModBus.ModbusMappingAddress.Invt_Ts" );
				}
				else
				{
