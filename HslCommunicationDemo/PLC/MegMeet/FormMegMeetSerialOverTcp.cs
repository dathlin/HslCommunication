...
{
    public partial class FormMegMeetSerialOverTcp : HslFormContent
    {
        ...
        {
            OperateResult<byte[]> read = megMeet.ReadFromCoreServer( HslCommunication.BasicFramework.SoftBasic.HexStringToBytes( textBox13.Text ) );
            if (read.IsSuccess)
            {
        ...
        private void thread_test2( )
        {
            ...
            while (count > 0)
            {
                if (!megMeet.Write( "100", (short)1234 ).IsSuccess) failed++;
                if (!megMeet.ReadInt16( "100" ).IsSuccess) failed++;
                count--;
            }
            thread_end( );
        }
