	...
	internal class Helper
	{
		...
		public static DeviceAddressExample[] GetMewtocolAddressExamples( )
		{
			return new DeviceAddressExample[]
			{
				...
				new DeviceAddressExample( "L2.1",      "链接继电器",     true, false, "L21 等同于 L2.1" ),
				new DeviceAddressExample( "D0 / DT0",  "数据寄存器 DT",  false, true, "D100 等同于 DT100, 读位使用 DM100.0" ),
				new DeviceAddressExample( "LD0",       "链接寄存器 LD",  false, true, "读位使用 LD100.0" ),
				new DeviceAddressExample( "F0",        "文件寄存器 FL",  false, true, "读位使用 F100.0" ),
