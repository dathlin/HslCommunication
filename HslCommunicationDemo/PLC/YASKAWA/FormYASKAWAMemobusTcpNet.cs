...
namespace HslCommunicationDemo
{
	public partial class FormYASKAWAMemobusTcpNet : HslFormContent
	{
		...
		private async void button_read_random_Click( object sender, EventArgs e )
		{
			// 随机字读取
			ushort[] address = textBox_read_random.Text.ToStringArray<ushort>( );
			button_read_random.Enabled = false;
			OperateResult<byte[]> read = await this.memobus.ReadRandomAsync( address );
			if (read.IsSuccess)
			{
			...
			button_read_random.Enabled = true;
		}
