...
namespace HslCommunicationDemo
{
	public partial class FormYokogawaLinkServer : HslFormContent
	{
		...
		private void FormSiemens_Load( object sender, EventArgs e )
		{
			...
			{
				Text = "YokogawaLink Virtual Server";
			}
		...
		private void button1_Click( object sender, EventArgs e )
		{
			try
			{
				...
				if (this.serverSettingControl1.ServerStart( yokoServer ) == false) return;
				...
				// 设置示例代码
				codeExampleControl.SetCodeText( "server", "", yokoServer, this.sslServerControl1 );
			}
			catch (Exception ex)
			{
		...
		{
			// 启动串口
			yokoServer.StartSerialSlave( this.serverSettingControl1.TextBox_Serial.Text );
			this.serverSettingControl1.ButtonSerial.Enabled = false;

			// 设置示例代码
			codeExampleControl.SetCodeText( "server", this.serverSettingControl1.TextBox_Serial.Text, yokoServer, this.sslServerControl1 );
